...
Some commonly used z-transform pairs are listed in Figure \ref{fig:L14_S16_1}.
...
In the case of LTI discrete-time systems we are concerned with in this course, all pertinent z-transforms are rational functions of \( z^{-1} \), that is, they are rations of two polinomials in \( z^{-1} \):
\begin{equation}
    G(z)
    =
    \frac{P(z)}{D(z)}
...
\begin{equation}
    ...
    \label{eq:L14_S18_1}
\end{equation}
Again, a rational z-transform can be alternately written in factored form as:
\begin{equation}
    G(z)
    =
...
We have as roots:
\begin{itemize}
    \item \( z = \xi_{\ell} \), roots of the numerator polynomial. These values of \( z \) are known as the zeros of \( G(z) \);
    \item \( z = \lambda_{\ell} \), roots of the denominator polynomial. These values of \( z \) are known as the poles of \( G(z) \).
\end{itemize}
...
\begin{example}{ROC of a rational z-transform}{}
...
A physical interpretation of the concepts of poles and zeros can be given by plotting the log-magnitude \( 20\log_{10}\abs{G(z)} \) as showed in Figure \ref{fig:L14_S25_1} for:
\begin{equation}
    G(z)
    =
    ...
    \label{eq:L14_S24_1}
\end{equation}
Observe that the magnitude plot exhibits very large peaks around the points \( z = 0.4 \pm j0.6928 \), which are the poles of \( G(z) \). It also exhibits very narrow and deep wells around the location of the zeros at \( z = 1.2 \pm j1.2 \).
...
ROC of a z-transform is an important concept. Without its knowledge, there is no unique relationship between a sequence and its z-transform. Hence, the z-transform must always be specified with its ROC. Moreover, there is a relationship between the ROC of the z-transform of the impulse response of a causal LTI discrete-time system and its BIBO stability.

Another important distiction is that a sequence can be one of the following types: finite-length, right-sided, left-sided and two-sided. In general, the ROC depends on the type of the sequence of interest.
...
In case \( G(z) \) has multiple poles, the partial-fraction expansion is of slightly different form. Let the pole at \( z = v \) be of multiplicity \( L \) and the remaining \( N - L \) poles be simple and at \( z = \lambda_{\ell} \), for \( 1 \le \ell \le N-L \).
...
\begin{figure}[!h]
    ...
    \caption{\label{fig:L14_S64_1} Properties of the z-transform.}
\end{figure}
